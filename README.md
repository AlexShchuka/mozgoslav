...
- Web-aware RAG (хождение в сеть) — не в v0.8, груминг в `.archive/adrs/ADR-008-web-rag.md`.
...
- `docs/adr/NEXT.md` — активная очередь работы (что делаем сейчас)
- `docs/adr/POSTRELEASE.md` — post-v1.0 задачи (DMG auto-update, Linux/Windows билды)
- `docs/adr/ADR-014-unrealized-backlog.md` — low-priority backlog
- `docs/adr/ADR-016-*.md`, `docs/adr/ADR-017-*.md` — Proposed ADR (RAG chat history, /metrics Prometheus)
- `CLAUDE.md` / `backend/CLAUDE.md` / `frontend/CLAUDE.md` / `python-sidecar/CLAUDE.md` — гиды для AI-агентов
- `.archive/adrs/` — shipped / superseded ADR (001…013, 015)
- `.archive/docs/v0.8-release/` — план релиза v0.8 поблочно (архив)
- `.archive/docs/backlog-cancelled-*.md` — cancelled items с обоснованием
