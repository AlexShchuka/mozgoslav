...
Никаких OKR, sprint planning, design committee, story points, retro. Решения = owner. Tests — там где ломается, coverage — пол, не цель.

Pet ≠ amateur sloppy. Pet = «острое маленькое craft», единый владелец со вкусом, без процессного жира.

Отпечаток: GitHub-native (issues + milestones + projects + dependencies) как единственный backlog-store; `agent-gate.sh` как единственный gate.
...
- Markdown export в Obsidian vault — event-driven, non-blocking: `ProcessQueueWorker` сохраняет ноту и публикует `ProcessedNoteSaved`; `ObsidianDomainEventConsumer` подхватывает и пишет файл через `IVaultDriver.WriteNoteAsync` в `{profile.ExportFolder ?? "_inbox"}/{date}-{topic}-{profile}.md`. Pipeline never blocks on Obsidian (ADR-019 §5.5 G1).
- Split-and-label через Templater: shipped через `obsidian-vault-bootstrap` (Wizard step 5 устанавливает `_system/scripts/split_and_label.js` + промпт + словарь корректировок + Templater pre-seed `data.json`). После того как нота попала в `_inbox/`, пользователь в Obsidian жмёт хоткей **«Templater: Insert split-and-label»** → `split_and_label.js` зовёт LM Studio и разбивает текст на блоки с лейблами **ИДЕЯ / ИНСАЙТ / ЧЕЛОВЕК / ВОПРОС / ЗАДАЧА** (FLAGGED → append в `_system/flagged.md`). Атомарные `.md` с frontmatter (`label`, `source`, `created`, `status: needs-review`, tags) ложатся в `ideas/`, `insights/`, `people/`, `questions/`, `tasks/`. Это и есть structured input для эффективной RAG (Столб 2: hybrid retrieval, metadata-filter по `label`, citations split by source-type). `_inbox/` — transit zone, не финал; пользователь сам переносит обработанный оригинал в `archive/` после ревью.
