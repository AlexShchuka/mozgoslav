...
FastAPI app with V3 ML endpoint stubs. Real ML work (diarization, gender, emotion, NER) is deferred until real models
are downloaded on a macOS host.
...
- `POST /api/embed` — real on macOS (sentence-transformers/all-MiniLM-L6-v2); falls back to a deterministic
  SHA-256-bucketed bag-of-words on dev boxes that cannot install PyTorch. Both modes return 384-dim L2-normalised
  vectors so the C# consumer never sees a contract change.
