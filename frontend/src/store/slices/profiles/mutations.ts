...
export const applyCreateOrReplace = (state: ProfilesState, profile: Profile): ByIdPatch => {
  const profiles = { ...state.profiles, [profile.id]: profile };
  const order = state.order.includes(profile.id) ? state.order : [...state.order, profile.id];
