...
describe("recordingReducer — live transcript", () => {
  it("LIVE_TRANSCRIPT_PARTIAL writes the entry for the given recordingId", () => {
    const state = reduce(
      ...
      liveTranscriptPartial({ recordingId: "rec-1", text: "hello", observedAt: "2024-01-01T00:00:00Z" })
    );
  ...
  it("LIVE_TRANSCRIPT_PARTIAL overwrites previous entry for same recordingId", () => {
    const after1 = reduce(
      initialRecordingState,
      liveTranscriptPartial({ recordingId: "rec-1", text: "first", observedAt: "2024-01-01T00:00:00Z" })
    );
    const after2 = reduce(
      after1,
      liveTranscriptPartial({ recordingId: "rec-1", text: "second", observedAt: "2024-01-01T00:00:01Z" })
    );
  ...
  it("multiple recordingIds coexist independently", () => {
    const s1 = reduce(
      initialRecordingState,
      liveTranscriptPartial({ recordingId: "rec-1", text: "aaa", observedAt: "2024-01-01T00:00:00Z" })
    );
    const s2 = reduce(
      s1,
      liveTranscriptPartial({ recordingId: "rec-2", text: "bbb", observedAt: "2024-01-01T00:00:00Z" })
    );
