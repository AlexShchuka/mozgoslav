...
const GroupedList = <T,>({
  ...
  const tree = useMemo(() => buildTree(items, getGroupPath ?? (() => null)), [items, getGroupPath]);
...
const BranchSection = <T,>({ node, depth, defaultCollapsed, ...rest }: BranchProps<T>) => {
  const [open, setOpen] = useState(!defaultCollapsed);
  const count = useMemo(() => countDescendantItems(node), [node]);

  return (
    <GroupSection $depth={depth}>
      <GroupHeader type="button" aria-expanded={open} onClick={() => setOpen((v) => !v)}>
        <Chevron $open={open}>
