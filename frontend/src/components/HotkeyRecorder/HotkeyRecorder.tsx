...
import { Root, Field, Placeholder, Value, Actions, Hint } from "./HotkeyRecorder.style";
...
// ------------------- Electron accelerator formatting ------------------- //
...
const buildAccelerator = (ev: KeyboardEvent): string | null => {
  ...
  if (keyName === null) {
    // Only modifier keys were pressed — wait for a real key.
    return null;
  }

  // Guard: require at least one modifier so users can't accidentally bind a
  // plain letter as a global hotkey and lock themselves out of typing.
...
const normaliseKey = (key: string, code: string): string | null => {
  ...
  // Function keys (F1…F24) — Electron accepts them verbatim.
  if (/^F\d{1,2}$/.test(key)) return key;
  // Letters — Electron wants upper-case.
  if (key.length === 1) return key.toUpperCase();
  // Digits via `code` (KeyboardEvent.key is different with Shift).
