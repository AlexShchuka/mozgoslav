...
describe("folderFromVaultPath", () => {
  ...
  it("supports nested folders", () => {
    expect(folderFromVaultPath("/vault/Projects/Client/a.md", "/vault")).toBe("Projects/Client");
  });
  ...
  it("normalizes Windows-style backslashes", () => {
    expect(folderFromVaultPath("C:\\vault\\Projects\\a.md", "C:\\vault")).toBe("Projects");
  });

  it("returns null if vault root does not match prefix (defensive)", () => {
    expect(folderFromVaultPath("/other/root/Archive/a.md", "/vault")).toBe("other/root/Archive");
  });
});
