  ...
  return (
    <PageRoot>
      <Toolbar>
        ...
        <Button variant="primary" onClick={onCreate} isLoading={creating} leftIcon={<Archive size={16} />}>
          {t("backup.create")}
        </Button>
      </Toolbar>
          ...
          {items.map((item) => (
            <BackupRow key={item.path}>
              <div>
                ...
                <BackupMeta>
                  {(item.sizeBytes / 1024 / 1024).toFixed(1)} MB · {new Date(item.createdAt).toLocaleString()}
                </BackupMeta>
              </div>
            </BackupRow>
          ))}
