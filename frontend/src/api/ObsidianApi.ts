...
export class ObsidianApi extends BaseApi {
  ...
  public async applyLayout(): Promise<ObsidianApplyLayoutResult> {
    const response = await this.post<ObsidianApplyLayoutResult>(API_ENDPOINTS.obsidianApplyLayout);
    return response.data;
  }
  ...
  public async reinstallPlugins(): Promise<ObsidianReinstallResult> {
    const response = await this.post<ObsidianReinstallResult>(
      API_ENDPOINTS.obsidianReinstallPlugins
    );
    return response.data;
  }
}
