...
public final class DictationHelper {
    ...
    public func run() {
        ...
        if let monitor = globalHotkeyMonitor {
            monitor.onEvent = { [weak self] payload in
                FileLog.shared.info("DictationHelper: emit global-hotkey kind=\(payload.kind)")
                self?.emit(event: "global-hotkey", params: .object([
                    "kind": .string(payload.kind),
                    "accelerator": .string(payload.accelerator),
                    "observedAt": .string(payload.observedAt),
                ]))
    ...
    private func dispatch(_ request: JsonRpcRequest) throws -> JsonRpcResponse {
        ...
        case "global-hotkey.register":
            let params = request.params?.objectValue() ?? [:]
            let id = params["id"]?.stringValue() ?? ""
            let key = params["key"]?.stringValue() ?? ""
            guard !id.isEmpty, !key.isEmpty else {
                throw HelperError(code: -32602, message: "global-hotkey.register requires id and key")
            }
            if let monitor = globalHotkeyMonitor {
                monitor.register(id: id, accelerator: key)
            } else {
                throw HelperError(code: -32601, message: "Global hotkey is only supported on macOS")
            }
            return JsonRpcResponse(id: request.id, result: .object(["registered": .bool(true)]))

        case "global-hotkey.unregister":
            let params = request.params?.objectValue() ?? [:]
            let id = params["id"]?.stringValue() ?? ""
            guard !id.isEmpty else {
                throw HelperError(code: -32602, message: "global-hotkey.unregister requires id")
            }
            if let monitor = globalHotkeyMonitor {
                monitor.unregister(id: id)
            }
            return JsonRpcResponse(id: request.id, result: .object(["unregistered": .bool(true)]))
...
public final class GlobalHotkeyMonitor {
    ...
    private var hotkeys = Dictionary<String, CGKeyCode>()
    private var listeners = Set<(HotkeyEvent) -> Void>()

    public init() {}

    public func register(id: String, accelerator: String): Bool {
        guard !id.isEmpty, !accelerator.isEmpty else { return false }
        
        let key = parseAccelerator(accelerator)
    ...
    public func unregister(id: String) {
        guard let key = hotkeys.remove(id) else { return }
        unregisterKey(key)
    }

    private func parseAccelerator(_ accelerator: String) -> CGKeyCode? {
        // Simplified parsing — in production use robust parser
        switch accelerator {
        case "Control+D": return 0x04 // 'D'
        default: return nil
        }
    }

    private func registerKey(_ key: CGKeyCode): Bool {
        guard let source = CGEventSource(stateID: .combinedSessionState) else { return false }
        
        var mask = CGEventModifiersMask(0)
        if accelerator.contains("Control") || accelerator.contains("Command") {
            mask |= .maskCommand
        }

        guard let eventDown = CGEvent(keyboardEventSource: source, virtualKey: key, keyDown: true),
              let eventUp = CGEvent(keyboardEventSource: source, virtualKey: key, keyDown: false) else { return false }
        
        eventDown.flags = mask
        eventUp.flags = mask

        // Register with uiohook-napi or native macOS APIs
        // For simplicity assume uiohook-napi is used elsewhere — here just log
        FileLog.shared.info("GlobalHotkeyMonitor: registered key=\(key)")
        return true
    }

    private func unregisterKey(_ key: CGKeyCode) {
        FileLog.shared.info("GlobalHotkeyMonitor: unregistered key=\(key)")
    }

    public var onEvent: ((HotkeyEvent) -> Void)? = nil

    // Simulate hotkey events — in production integrate with actual event loop
    private func emit(kind: String, accelerator: String) {
        guard let listener = onEvent else { return }
        listener(HotkeyEvent(
            kind: kind,
            accelerator: accelerator,
            observedAt: NSDate().description
        ))
    }
}
