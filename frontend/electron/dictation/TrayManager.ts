...
export class TrayManager {
  ...
  private solidPng16(colour: [number, number, number]): Buffer {
    // Minimal 16x16 RGBA PNG built on the fly. Matches what electron expects
    // for `createFromBuffer`.
    const width = 16;
    const height = 16;
...
// Pure-TS PNG encoder kept small enough for a solid-colour icon. Stdlib `zlib`
// handles the IDAT deflate; no external deps.
