...
function playCue(cue: SoundCue): void {
  ...
  try {
    ...
    child.on("error", () => {
      // Missing binary / missing sound file — treat as silent no-op so the
      // dictation pipeline never trips over audio setup problems.
    });
    child.unref();
  } catch {
    // Spawn itself can throw on sandboxed / restricted environments; the UX
    // goal is audible-when-possible, never-block-otherwise.
  }
}
