...
Единый язык в стиле Logi Options+ 2026: глубокие чёрно-синие фоны, живой cyan/mint как единственный акцент, motion-дизайн как подсказка действия (не декоратив). Один источник токенов — `frontend/src/styles/theme.ts`.
...
- `styles/motion.ts` (новый) — framer-motion variants экспорт (`buttonVariants`, `cardVariants`, `sidebarVariants`, etc).
